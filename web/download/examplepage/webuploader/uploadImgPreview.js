...
(function($) {
    ...
    function removeInitImage(file, percentages) {
        var $li = $('#' + file.id);
        delete percentages[file.id];
        $li.find('.file-panel').off().end().remove();
        $li.off();
        options.onDelete && $.isFunction(options.onDelete) && options.onDelete();
    }
    /*设置上传状态*/
    ...
    function uploadImage(option) {
        ...
        uploader.onInitImg = function(src, _id) {
            var fileid = src.substring(src.length - 15);
            var file = {
                "id": _id,
                "name": fileid,
                "statusText": "valid",
            ...
            // var image = new Image();
            // image.src = src;
            // // // image.resize( 120, options.height );
            ...
            var $li = $('<li id="' + file.id + '">' + '<p class="title">' + file.name + '</p>' + '<p class="imgWrap"></p>' + '<p class="progress"><span></span></p>' + '</li>'),
                ...
                $prgress = $li.find('p.progress span'), //上传进度条
                $wrap = $li.find('p.imgWrap'), //包裹图片的元素
                $info = $('<p class="info"></p>'); //上传错误信息
            var img = $('<img width="' + wi + '" src="' + src + '">');
            $info.html("已有图片").appendTo($li);
            //预览图片垂直水平显示
            if(options.viewImgHorizontal){
                img.one( 'load', function() {
