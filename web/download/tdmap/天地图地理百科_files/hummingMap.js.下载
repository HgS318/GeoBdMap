...
define(function(require){
    ...
    function initmap(){
        ...
        var zoom = 15;
        //初始化地图对象
        _map = new TMap("map",{projection:"EPSG:4326"});

        /*var maskCfg = {};
        maskCfg.getTileUrl = function(x,y,z){
            return "images/mask.png";
        }
        //创建自定义图层对象
        var masklay = new TTileLayer(maskCfg);
        masklay.setGetTileUrl(maskCfg.getTileUrl);
        //将图层增加到地图上
        _map.addLayer(masklay);*/

        //设置显示地图的中心点和级别
        _map.centerAndZoom(new TLngLat(109.5131,18.77689),zoom);
        ...
        // 添加地图图层菜单
        /*var mapMenuObj = document.getElementById("hmmapmenu");
        if (mapMenuObj != null){
            var mapMenuControl = new THtmlElementControl(mapMenuObj);
            mapMenuControl.setLeft(70);
            mapMenuControl.setTop(10);
            _map.addControl(mapMenuControl);
        }*/
    ...
    function onHeadClick(e){
        ...
        if (_preHeadOid != null){
            var preHeadObj = document.getElementById(_preHeadOid);
