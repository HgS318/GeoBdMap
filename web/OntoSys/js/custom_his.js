...
$(document).ready(function () {
	$('#id_tree').tree({
		lines: true,
		animate: false,
		url: 'http://localhost/btk/s.asmx/getData?',
		queryParams: { id: '' },
	...
	$('#id_searchresult').datagrid({
		...
		width: '100%',
		height: '100%',
		url: 'http://localhost/btk/s.asmx/getSearch',
...
function V(fldm) {	//查看范例的全部内容
	...
	$.ajax({
		...
		async: true,
		contentType: "application/json;charset=utf-8",
		url: "http://localhost/btk/s.asmx/getDetail?jsoncallback=?",
		data: { fldm: fldm },
		dataType: "jsonp",
		timeout: 5000,
		jsonpCallback: 'jsoncallback',
...
function ax(id, title, jsonData) {	//展示图片 id:对应的范例种类标号, title：范例种类, jsonData：各图像的url
	...
	if (jsonData.length > 0) {
		...
		for (i = 0; i < jsonData.length; i++) {
			htmlstr += '<li class="' + id + '_' + i + '"><img src="' + jsonData[i].url +
				'" alt="样例' + (i + 1) + '" /></li>';
		}
		htmlstr += '</ul>';
		var T = $(htmlstr);
...
function bN(id, title, jsonData) {	//	展示全景
    var l = $('<div class="griditem" style="width: ' + G + '; height: ' +
